...
@click.option(
    ...
    "-r",
    default=None,
    help="JSON parameter overrides: '{\"gov.tax.rate\": 0.25}'",
)
@click.option(
    "--strip-comments",
    is_flag=True,
...
def main(
    ...
    if reform:
        ...
        try:
            ...
            param_values = apply_reform_to_parameters(
                param_values, reform_values
            )
            click.echo(
                f"Applied {len(reform_values)} reform override(s)",
                err=True,
            )
...
Parameters: {len(param_values)}{reform_info}
"""
